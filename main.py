...
rot = (len(move) % q) * 8
mixer.init()
colour_scheme = [(255, 255, 255), (128, 128, 128), (0, 0, 0)]
...
def possible_bishop(selection):
    a = []
    b = []
    for i in range(5 - selection.r):
        b.append([(selection.a + i + 1) % 24, selection.r + i + 1])
    a.append(b)
    b = []
    for i in range(5 - selection.r):
        b.append([(selection.a - i - 1) % 24, selection.r + i + 1])
...
def check_king(selection):
    a = []
    a = possible_queen(selection)
    a.append(possible_knight(selection))
    return a
...
class Piece(pygame.sprite.Sprite):
    ...
    def check(self, selection):
        for i in range(len(selection)):
            for j in range(len(selection[i])):
                potential_check = [s for s in pieces if [s.a, s.r] == selection[i][j]]
                if not potential_check:
                    continue
                elif potential_check[0].colour == self.colour:
                    break
                elif not potential_check[0].colour == self.colour:
                    if i <= 3:
                        if potential_check[0].rank in [3, 4]:
                            show_text("check")
                    elif i <= 7:
                        if potential_check[0].rank in [2, 4]:
                            show_text("check")
                    elif i == 8:
                        if potential_check[0].rank == 1:
                            show_text("check")
    ...
    def control_linear(self, a, r, path):
        ...
        touch_sprites = []
        moat = 0
        if [a, r] in path and not [a, r] == [self.a, self.r]:
            print(path[0: (path.index([a, r]))])
            for j in range(path.index([a, r]) + 1):
                if path[j] in [[1, 4], [9, 4], [17, 4], [1, 5], [9, 5], [17, 5], [2, 4], [10, 4],
                               [18, 4], [2, 5], [10, 5], [18, 5]]:
                    moat = 1
                print("j=", j)
                ...
                if path.index([a, r]) == j:
                    ...
                    if not touch_sprites[j]:
                        ...
                        self.a = a
                        self.r = r
                    elif not touch_sprites[j][0].colour == self.colour and (
                            ((touch_sprites[j][0].a - 2) % 24) // 8 == ((self.a - 2) % 24) // 8 or (
                            not ((touch_sprites[j][0].a - 2) % 24) // 8 == ((self.a - 2) % 24) // 8) and moat == 0):
                        print(((touch_sprites[j][0].a + 2) % 24) // 8, ((self.a + 2) % 24) // 8)
                        print("b happened")
    ...
    def control(self, a, r):
        ...
        if not touch_sprites == [] and not [a, r] == [self.a, self.r]:
            for i in range(len(touch_sprites)):
                if not self.colour == touch_sprites[i].colour:
    ...
    def move_pawn(self, selection, a, r):
        ...
        if not touched_sprites2 and [a, r] in selection and not [a, r] == [self.a, self.r]:
            move.append([[a, r], [self.a, self.r]])
            # ws.send(str(mov))
            self.rect.center = (c[a][r][0], c[a][r][1])
            self.a = a
    ...
    def cut_pawn(self, selection, a, r):
        ...
        for i in range(len(touched_sprites)):
            ...
            print(i, "cuttable")
        for j in range(2):
            if [a, r] == [touched_sprites[i].a, touched_sprites[i].r] and not [a, r] == [self.a, self.r]:
...
Piece_index_list = [
    [[18, 5], [19, 5], [20, 5], [21, 5], [22, 5], [23, 5], [0, 5], [1, 5], [18, 4], [19, 4], [20, 4], [21, 4],
     [22, 4], [23, 4], [0, 4], [1, 4]],
    [[10, 5], [11, 5], [12, 5], [13, 5], [14, 5], [15, 5], [16, 5], [17, 5], [10, 4], [11, 4], [12, 4], [13, 4],
     [14, 4], [15, 4], [16, 4], [17, 4]],
    [[2, 5], [3, 5], [4, 5], [5, 5], [6, 5], [7, 5], [8, 5], [9, 5], [2, 4], [3, 4], [4, 4], [5, 4], [6, 4],
     [7, 4], [8, 4], [9, 4]]]
...
def create_button(x, y, width, height, hovercolor, defaultcolor, msg, player1='', player2='', player3=''):
    ...
    if x + width > mouse[0] > x and y + height > mouse[1] > y:
        ...
        pygame.draw.rect(screen, defaultcolor, (x, y, width, height), border_top_left_radius=3,
    ...
    if msg == "Start":
        ...
        screen.blit(buttontext,
                    (int((screen.get_width() - buttontext.get_width()) / 2), 577.5 - buttontext.get_height() / 2))
    elif msg == "Back":
        buttontext = smallfontbold.render(msg, True, white)
...
def next_rules():
    ...
    while next_rules_running:
        screen.fill(ceramic)
        next_rules_backbutton = create_button(x=7, y=7, width=80, height=26, hovercolor=light_wood,
                                              defaultcolor=dark_wood, msg="Back")

        rules10 = [helpfontbold.render('10.', True, dark_wood),
                   mediumfont.render("If either order of a knight's move would carry it horizontally across a", True,
                                     dark_wood),
                   mediumfont.render("moat. Without that knight being able to bridge the moat then that", True,
                                     dark_wood),
                   mediumfont.render("move is illegal.", True, dark_wood)]

        rules11 = [helpfontbold.render('11.', True, dark_wood),
                   mediumfont.render("A team's checkmate status is not finalized until it is their turn unless", True,
                                     dark_wood),
                   mediumfont.render("their king is captured.", True, dark_wood)]
        rules12 = [helpfontbold.render('12.', True, dark_wood),
                   mediumfont.render("When a player is checkmated that player is eliminated from the game", True,
                                     dark_wood),
                   mediumfont.render("but all their pieces remain on the board these pieces do not move.", True,
                                     dark_wood)]
        rules13 = [helpfontbold.render('13.', True, dark_wood),
                   mediumfont.render("If you wish to occupy a space with an eliminated player's piece on it", True,
                                     dark_wood),
                   mediumfont.render("then you must capture that piece.", True, dark_wood)]
        rules14 = [helpfontbold.render('14.', True, dark_wood),
                   mediumfont.render("If a player cannot make a legal move called a stalemate and three", True,
                                     dark_wood),
                   mediumfont.render("players are still in then that player is considered checkmated.", True,
                                     dark_wood)]
        rules15 = [helpfontbold.render('15.', True, dark_wood),
                   mediumfont.render("If there are only two players during a stalemate then the game is", True,
                                     dark_wood),
                   mediumfont.render("a draw otherwise the game continues until only one player remains.", True,
                                     dark_wood)]
...
def rules():
    ...
    while rules_running:
        screen.fill(ceramic)
        rules_backbutton = create_button(x=7, y=7, width=80, height=26, hovercolor=light_wood, defaultcolor=dark_wood,
                                         msg="Back")
        next_button = create_button(x=713, y=7, width=80, height=26, hovercolor=light_wood, defaultcolor=dark_wood,
                                    msg="Next")
        ...
        rules1 = [helpfontbold.render('1.', True, dark_wood),
                  mediumfont.render('The object of the game is to checkmate both opponents kings.', True, dark_wood)]

        rules2 = [helpfontbold.render('2.', True, dark_wood),
                  mediumfont.render('The center of the board may be passed through. It is not a space', True,
                                    dark_wood),
                  mediumfont.render('that can be occupied. Pieces may move straight to the other side.', True,
                                    dark_wood)]

        rules3 = [helpfontbold.render('3.', True, dark_wood),
                  mediumfont.render('To move a piece diagonally through the center follow the trajectory', True,
                                    dark_wood),
                  mediumfont.render('lines out from the square to the other side.', True, dark_wood)]

        rules4 = [helpfontbold.render('4.', True, dark_wood),
                  mediumfont.render('Knights can move two squares vertically then one square horizont-', True,
                                    dark_wood),
                  mediumfont.render('ally or one square vertically then two squares horizontally.', True, dark_wood)]

        rules5 = [helpfontbold.render('5.', True, dark_wood),
                  mediumfont.render('Moats are the thick green lines that border each team on the outer', True,
                                    dark_wood),
                  mediumfont.render("row. No piece can check an opponent's king through a moat.", True, dark_wood)]

        rules6 = [helpfontbold.render('6.', True, dark_wood),
                  mediumfont.render('No piece may travel across a moat horizontally or diagonally unless', True,
                                    dark_wood),
                  mediumfont.render('the moat is bridged. ', True, dark_wood)]

        rules7 = [helpfontbold.render('7.', True, dark_wood),
                  mediumfont.render("A piece may momentarily bridge a moat if its move doesn't result in", True,
                                    dark_wood),
                  mediumfont.render("the capture of a piece or puts an opponent's king in check.", True, dark_wood)]

        rules8 = [helpfontbold.render('8.', True, dark_wood),
                  mediumfont.render('If a player is eliminated then both of its moats are permanently', True,
                                    dark_wood),
                  mediumfont.render('bridged for all pieces.', True, dark_wood)]

        rules9 = [helpfontbold.render('9.', True, dark_wood),
                  mediumfont.render('Once an active player vacates their outer row, even if they bring p-', True,
                                    dark_wood),
...
def game():
    ...
    while game_running:
        screen.fill(ceramic)
        game_backbutton = create_button(x=7, y=7, width=80, height=26, hovercolor=light_wood, defaultcolor=dark_wood,
                                        msg="Back")
        if game_backbutton:
            game_running = False
        ...
        # check_king([s for s in pieces if s.get_rank() == "king" and s.colour == len(move) % 3][0])
        # [s for s in pieces if s.get_rank() == "king" and s.colour == len(move) % 3][0].check(check_king([s for s in pieces if s.get_rank() == "king" and s.colour == len(move) % 3][0]))
        pygame.draw.circle(screen, colour_scheme[(len(move) + 1) % 3], (400, 400), 30)
        pygame.draw.circle(screen, colour_scheme[len(move) % q], (400, 400), 25)
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN and not selected:
                try:
                    ...
                    if live_colour[len(move) % len(live_colour)] == piece_selected.colour:
                        selected = True
                ...
                if piece_selected.get_rank() == "bishop":
                    for i in range(len(possible_bishop(piece_selected))):
                        if [coordangle, d] in possible_bishop(piece_selected)[i]:
                            piece_selected.control_linear(coordangle, d, possible_bishop(piece_selected)[i])
                ...
                if piece_selected.get_rank() == "pawn":
                    if [coordangle, d] in possible_pawn(piece_selected)[0]:
                        ...
                        if piece_selected.r == 5:
                            ...
                            piece_selected.image = \
                                [s for s in pieces if s.colour == piece_selected.colour and piece_selected.rank == 6][
                                    0].image
...
def menu():
    ...
    while running:
        ...
        backbutton = create_button(x=7, y=7, width=80, height=26, hovercolor=light_wood, defaultcolor=dark_wood,
                                   msg="Back")
        helpbutton = create_button(x=713, y=767, width=80, height=26, hovercolor=blue_wood, defaultcolor=help_wood,
                                   msg="Rules")
        playbutton = create_button(x=713, y=7, width=80, height=26, hovercolor=light_wood, defaultcolor=dark_wood,
                                   msg="Play")
        ...
        player1Border = pygame.Rect(((500 - player1Surface.get_width()) / 2) - 5, screen_height * .10 - 10,
                                    player1Surface.get_width() + 10, 50)

        player2Surface = mediumfontbold.render(player2, True, another_dark_wood)
        player2Border = pygame.Rect(((500 - player2Surface.get_width()) / 2) - 5, 290, player2Surface.get_width() + 10,
                                    50)

        player3Surface = mediumfontbold.render(player3, True, another_dark_wood)
        player3Border = pygame.Rect(((500 - player3Surface.get_width()) / 2) - 5, 510, player3Surface.get_width() + 10,
                                    50)
        ...
        enterbutton = create_button(x=400 - 50, y=710, width=100, height=50, hovercolor=light_wood,
                                    defaultcolor=dark_wood, msg="Enter", player1=player1, player2=player2,
                                    player3=player3)
        ...
        screen.blit(player1Prompt,
                    ((500 - player1Prompt.get_width()) / 2, (screen_height * .10) + player1Prompt.get_height()))
...
def start_menu():
    ...
    credit = [creditfont.render("Made by: ", True, credit_color),
              creditfont.render("Ashesh Mishra", True, credit_color),
              creditfont.render("Riddhiman Sengupta", True, credit_color),
              creditfont.render("Rachit Kumar", True, credit_color),
              creditfont.render("Soham Chakraborty", True, credit_color)]
    ...
    while True:
        ...
        startbutton = create_button(x=(screen.get_width() - 300) / 2, y=400, width=300, height=100,
                                    hovercolor=light_wood, defaultcolor=dark_wood, msg="Start")
        leavebutton = create_button(x=(screen.get_width() - 125) / 2, y=552.5, width=125, height=50,
                                    hovercolor=lightred_wood, defaultcolor=dark_wood, msg="Leave")
